...
export default function ActivityList({ activities, dispatch }: ActivityListProps) {
    ...
    return (
        <>
            ...
            {
            isEmptyActivities ? 
            <p className="text-center text-2xl my-5"> No hay actividades aun... </p> :
                ...
                <div key={activity.id}
                    ...
                    <div className="space-y-2 relative">
                        <p className={`absolute -top-8 -left-8 px-10 py-2 text-white uppercase font-bold 
                            ${activity.category === 1 ? 'bg-teal-500' : 'bg-red-500'}`}>
                            {categoryName(activity.category)}
                        </p>
                        <p className="text-2xl font-bold pt-5">{activity.name}</p>
                        <p className={`text-4xl font-black ${activity.category === 1 ? 'text-teal-500' : 'text-red-500'} `}>{activity.calories}{''} <span>Calorias</span> </p>
                    </div>
